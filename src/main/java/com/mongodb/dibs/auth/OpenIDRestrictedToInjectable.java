...
/**
 * <p>Injectable to provide the following to {@link OpenIDRestrictedToProvider}:</p> <ul> <li>Performs decode from HTTP request</li>
 * <li>Carries OpenID authentication data</li> </ul>
 *
 * @since 0.0.1
 */
class OpenIDRestrictedToInjectable<T> extends AbstractHttpContextInjectable<T> {
    ...
    public T getValue(HttpContext httpContext) {
        ...
        try {
            ...
            if (!cookieMap.containsKey(DibsConfiguration.SESSION_TOKEN_NAME)) {
                throw new WebApplicationException(Response.Status.UNAUTHORIZED);
            }

            UUID sessionToken = UUID.fromString(cookieMap.get(DibsConfiguration.SESSION_TOKEN_NAME).getValue());
